 ...
 */
typedef struct {
	/* mutex for accessing axon pro registers */
	pthread_mutex_t registers_mutex;
	/* variable to hold mutex wait result */
	int mutex_wait_result;
	/* binary semaphore to simulate axon pro interrupts */
	sem_t *semaphore;
	/* thread for axon_pro hardware */
	pthread_t hw_thread_handle;
	/* thread for axon_pro int */
	pthread_t int_thread_handle;
		...
		NRF_AXON_PLATFORM_BITWIDTH_UNSIGNED_TYPE value)
{
	if (nrf_axon_simulator_in_threadless_mode_get()) {
		...
		case 0:
			break;
		case 1:
			if (axon_dsp_simulator_process_action_request(
					&dsp_simulator_err, &dsp_o_cycles,
	...
	volatile NRF_AXON_PLATFORM_BITWIDTH_UNSIGNED_TYPE *addr)
{
	...
	if (nrf_axon_simulator_in_threadless_mode_get()) {
		...
		return_value = axon_dsp_simulator_read_reg_prim(addr);
	} else {
		/* grab mutex by calling wait() */
		simulator_state.axon_dsp.mutex_wait_result =
			pthread_mutex_lock(&simulator_state.axon_dsp.registers_mutex);
...
void *axon_dsp_hw_thread(void *data)
{
	...
	while (1) {
		...
		if (simulator_state.axon_dsp.action_reg_updated == 0) {
			/* nothing yet, skip below and go back to the beginning of while loop */
			continue;
		}
...
static int start_simulator_axon_dsp(void)
{
	...
	if (!nrf_axon_simulator_in_threadless_mode_get()) {
		...
		if (ret != 0) {
			printf("AXON DSP Create Mutex error: %d\n", ret);
			return -1;
		}
		/* create semaphore */
		simulator_state.axon_dsp.semaphore =
			sem_open("/axon_dsp_semaphore", O_CREAT, 0644, 0);
		if (SEM_FAILED == simulator_state.axon_dsp.semaphore) {
			printf("AXON DSP Create Semaphore error: %d\n", errno);
			system("PAUSE");
		}

		/* create interrupt thread semaphore */
		simulator_state.axon_dsp.terminate_int_thread_semaphore =
			sem_open("/axon_dsp_term_int_thread_sem", O_CREAT, 0644, 0);
		if (SEM_FAILED == simulator_state.axon_dsp.terminate_int_thread_semaphore) {
			printf("AXON DSP Create Semaphore interrupt thread terminate error: %d\n",
				errno);
			system("PAUSE");
		}
		...
		ret = pthread_create(&simulator_state.axon_dsp.int_thread_handle, NULL,
			axon_dsp_int_thread, NULL);
		if (ret != 0) {
...
void exit_simulator_axon_dsp(void)
{
	if (!nrf_axon_simulator_in_threadless_mode_get()) {
		...
		/* release semaphore to cause interrupt thread to exit cleanly */
		...
		sem_close(simulator_state.axon_dsp.terminate_int_thread_semaphore);
		sem_close(simulator_state.axon_dsp.semaphore);
	}
}
		...
		NRF_AXON_PLATFORM_BITWIDTH_UNSIGNED_TYPE value)
{
	if (nrf_axon_simulator_in_threadless_mode_get()) {
		...
		case 0:
			break;
		case 1:
			if (axon_nn_simulator_process_action_request(
					&nn_simulator_err, &nn_o_cycles,
	...
	volatile NRF_AXON_PLATFORM_BITWIDTH_UNSIGNED_TYPE *addr)
{
	...
	if (nrf_axon_simulator_in_threadless_mode_get()) {
		/* read value from application register set */
		return_value = axon_nn_simulator_read_reg_prim(addr);
	} else {
		/* grab mutex by calling wait() */
		simulator_state.axon_nn.mutex_wait_result =
			pthread_mutex_lock(&simulator_state.axon_nn.registers_mutex);
...
static void *start_simulator_axon_nn(void)
{
	...
	if (!nrf_axon_simulator_in_threadless_mode_get()) {
		...
		if (ret != 0) {
			printf("AXON NN Create Mutex error: %d\n", ret);
			return 0;
		}

		/* create semaphore */
		simulator_state.axon_nn.semaphore =
			sem_open("/axon_nn_semaphore", O_CREAT, 0644, 0);
		if (SEM_FAILED == simulator_state.axon_nn.semaphore) {
			printf("AXON NN Create Semaphore error: %d\n", errno);
			system("PAUSE");
		}

		/* create interrupt thread semaphore */
		simulator_state.axon_nn.terminate_int_thread_semaphore =
			sem_open("/axon_nn_term_int_thread_sem", O_CREAT, 0644, 0);
		if (SEM_FAILED == simulator_state.axon_nn.terminate_int_thread_semaphore) {
			printf("AXON NN Create Semaphore interrupt thread terminate error: %d\n",
				errno);
			system("PAUSE");
		}
...
void exit_simulator_axon_nn(void)
{
	if (!nrf_axon_simulator_in_threadless_mode_get()) {
		...
		sem_close(simulator_state.axon_nn.terminate_int_thread_semaphore);
		sem_close(simulator_state.axon_nn.semaphore);
	}
}
...
{
	static_assert(
		sizeof(void *) == sizeof(NRF_AXON_PLATFORM_BITWIDTH_UNSIGNED_TYPE),
...
int fprintf_s(FILE * stream,const char * format, ...) {
  ...
  va_end(args);
	return fprintf(stream, "%s", tempstring);  /* the "%s" silences a clang warning */
}
...
int nrf_axon_simulator_run_test_files(
  ...
  int (*callback_function)(char* input_file_name, char* output_file_name, int8_t* buffer, uint32_t buffer_size)) {
	...
	char forwardslash[2] = "/"; /* A string containing the backslash character */

  struct dirent *entry;
  if((input_file_path[strnlen(input_file_path, STR_BUF_SIZE)-1] != '\\') && (input_file_path[strnlen(input_file_path, STR_BUF_SIZE)-1] != '/')) {
    snprintf(fullPath_input, sizeof(fullPath_input), "%s%s", input_file_path, forwardslash);
  } else {
  ...
  while ((entry = readdir(dp))) {
      if (strstr(entry->d_name, input_file_ext)) {
          ...
          if((input_file_path[strnlen(input_file_path, STR_BUF_SIZE)-1] != '\\') && (input_file_path[strnlen(input_file_path, STR_BUF_SIZE)-1] != '/')) {
            snprintf(fullPath_input, sizeof(fullPath_input), "%s%s%s", input_file_path, forwardslash, entry->d_name);
          } else {
          ...
          if((output_file_path[strnlen(output_file_path, STR_BUF_SIZE)-1] != '\\') && (output_file_path[strnlen(output_file_path, STR_BUF_SIZE)-1] != '/')) {
            snprintf(fullPath_output, sizeof(fullPath_output), "%s%s%s%s", output_file_path, forwardslash, output_file_head_str, entry->d_name);
          } else {
