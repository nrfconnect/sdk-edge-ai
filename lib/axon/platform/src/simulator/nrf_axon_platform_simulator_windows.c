...
typedef struct {
	/* mutex for accessing axon nn registers */
	HANDLE registers_mutex;
	/* variable to hold mutex wait result */
	DWORD mutex_wait_result;
	/* binary semaphore to simulate axon nn interrupts */
	HANDLE semaphore;
	/* thread for cu hardware */
	HANDLE hw_thread_handle;
	/* thread for cu int */
	HANDLE int_thread_handle;
	HANDLE terminate_int_thread_semaphore;
	volatile bool terminate_thread;
	/* indicates that user has modified a command or sticky bit register */
	volatile uint8_t action_reg_updated;
	volatile int action_processed_cnt;
} axon_cu_state_s;
typedef struct {
	axon_cu_state_s axon_dsp;
	axon_cu_state_s axon_nn;
} axon_simulator_state_s;
static axon_simulator_state_s simulator_state;

/*
 * Axon write to register(executes in the application thread only)
 */
void axon_dsp_simulator_write_reg(volatile NRF_AXON_PLATFORM_BITWIDTH_UNSIGNED_TYPE *addr,
	NRF_AXON_PLATFORM_BITWIDTH_UNSIGNED_TYPE value)
{
	if (nrf_axon_simulator_in_threadless_mode_get()) {
		...
		case 0:
			break;
		case 1:
			if (axon_dsp_simulator_process_action_request(&dsp_simulator_err,
					&dsp_o_cycles, &dsp_o_wdog_cmd, &dsp_o_wdog_finish)) {
		...
		simulator_state.axon_dsp.mutex_wait_result =
			WaitForSingleObject(simulator_state.axon_dsp.registers_mutex,
				INFINITE);
 ...
 */
NRF_AXON_PLATFORM_BITWIDTH_UNSIGNED_TYPE axon_dsp_simulator_read_reg(
	volatile NRF_AXON_PLATFORM_BITWIDTH_UNSIGNED_TYPE *addr)
{
	if (nrf_axon_simulator_in_threadless_mode_get()) {
		...
		NRF_AXON_PLATFORM_BITWIDTH_UNSIGNED_TYPE return_value =
			axon_dsp_simulator_read_reg_prim(addr);
		return return_value;
	}
	simulator_state.axon_dsp.mutex_wait_result =
		WaitForSingleObject(simulator_state.axon_dsp.registers_mutex, INFINITE);
...
bool axon_dsp_simualtor_int_pending(void)
{
	if (WAIT_OBJECT_0 !=
		WaitForSingleObject(simulator_state.axon_dsp.registers_mutex, INFINITE)) {
		printf("AXON DSP Mutex wait failed..");
...
DWORD WINAPI axon_dsp_int_thread(void* data) {
	...
	HANDLE interrupt_semaphores[2] = {
		simulator_state.axon_dsp.terminate_int_thread_semaphore,
		simulator_state.axon_dsp.semaphore};

	while (1) {
		DWORD semaphore_wait_result;

		/*
		 * wait infinite for semaphores until an int has fired.
		 * Then only peek at the terminate semaphore
		 */
		semaphore_wait_result =
			WaitForMultipleObjects(2-int_fired, interrupt_semaphores, FALSE,
			int_fired ? 0 : INFINITE);
		...
		/*
		 * allow for the int firing while disabled, then the int is cleared. Need to
		 * suppress the ISR in that case.
		 */
 ...
 */
static void start_simulator_axon_dsp(void)
{
	...
	if (!nrf_axon_simulator_in_threadless_mode_get()) {
		 ...
		 */
		simulator_state.axon_dsp.hw_thread_handle = CreateThread(NULL, 0,
			axon_dsp_hw_thread, NULL, 0, &hw_thread_id);
		if (simulator_state.axon_dsp.hw_thread_handle == NULL) {
			printf("AXON HW Thread creation failed. Exiting program\n");
			fflush(stdout);
			system("PAUSE");
		}
		WaitForSingleObject(simulator_state.axon_dsp.hw_thread_handle, 1000);
		simulator_state.axon_dsp.int_thread_handle =
			CreateThread(NULL, 0, axon_dsp_int_thread, NULL, 0, &int_thread_id);
		if (simulator_state.axon_dsp.int_thread_handle == NULL) {
			printf("AXON DSO INT Thread creation failed. Exiting program\n");
			fflush(stdout);
			system("PAUSE");
		}
		WaitForSingleObject(simulator_state.axon_dsp.int_thread_handle, 1000);
	}
}

void exit_simulator_axon_dsp(void)
{
	if (nrf_axon_simulator_in_threadless_mode_get()) {
		return;
	}
	...
	DWORD wait_result =
		WaitForSingleObject(simulator_state.axon_dsp.hw_thread_handle, INFINITE);
	...
	/*
	 * for the interrupt thread, release a semaphore which will then
	 * cause it to exit cleanly
	 */
	ReleaseSemaphore(simulator_state.axon_dsp.terminate_int_thread_semaphore, 1, 0);
	wait_result = WaitForSingleObject(simulator_state.axon_dsp.int_thread_handle,
		INFINITE);
	...
	NRF_AXON_PLATFORM_BITWIDTH_UNSIGNED_TYPE value)
{
	...
	simulator_state.axon_nn.mutex_wait_result =
		WaitForSingleObject(simulator_state.axon_nn.registers_mutex, INFINITE);
...
bool axon_nn_simualtor_int_pending(void)
{
	if (WAIT_OBJECT_0 !=
			WaitForSingleObject(simulator_state.axon_nn.registers_mutex, INFINITE)) {
		printf("AXON NN Mutex wait failed..");
...
DWORD WINAPI axon_nn_int_thread(void *data)
{
	...
	HANDLE interrupt_semaphores[2] = {simulator_state.axon_nn.terminate_int_thread_semaphore,
		simulator_state.axon_nn.semaphore};
	while (1) {
		DWORD semaphore_wait_result;
		/*
		 *wait infinite for semaphores until an int has fired.
		 * Then only peek at the terminate semaphore
		 */
		semaphore_wait_result = WaitForMultipleObjects(2-int_fired,
			interrupt_semaphores, FALSE, int_fired ? 0 : INFINITE);
...
DWORD WINAPI axon_nn_hw_thread(void *data)
{
	...
	while (1) {
		...
		if (axon_nn_simulator_process_action_request(&nn_simulator_err, &nn_o_cycles,
				&nn_o_wdog_cmd, &nn_o_wdog_finish)) {
...
static void *start_simulator_axon_nn(void)
{
	...
	if (!nrf_axon_simulator_in_threadless_mode_get()) {
		...
		simulator_state.axon_nn.terminate_int_thread_semaphore =
			CreateSemaphore(NULL, 0, 1, NULL);
		 ...
		 * EC  - find where stack size is being set/overwritten in exe
		 */
		simulator_state.axon_nn.hw_thread_handle =
			CreateThread(NULL, 0, axon_nn_hw_thread, NULL, 0, &hw_thread_id);
		...
		if (!SetThreadPriority(simulator_state.axon_nn.hw_thread_handle,
				THREAD_PRIORITY_ABOVE_NORMAL)) {
			printf("AXON NN HW Thread priority set failed.\n");
		}


		WaitForSingleObject(simulator_state.axon_nn.hw_thread_handle, 1000);
		simulator_state.axon_nn.int_thread_handle =
			CreateThread(NULL, 0, axon_nn_int_thread, NULL, 0, &int_thread_id);
		if (simulator_state.axon_nn.int_thread_handle == NULL) {
			printf("AXON NN INT Thread creation failed. Exiting program\n");
			fflush(stdout);
			system("PAUSE");
		}
		WaitForSingleObject(simulator_state.axon_nn.int_thread_handle, 1000);
	}

	return result;
}

void exit_simulator_axon_nn(void)
{
	if (nrf_axon_simulator_in_threadless_mode_get()) {
		return;
	}
	...
	DWORD wait_result =
		WaitForSingleObject(simulator_state.axon_nn.hw_thread_handle, INFINITE);
	if (wait_result == WAIT_OBJECT_0) {
	...
	wait_result = WaitForSingleObject(simulator_state.axon_nn.int_thread_handle, INFINITE);
	/* and if that is successful close the interrupt thread handle */
	if (wait_result == WAIT_OBJECT_0) {
		...
		int8_t *input_vector, uint32_t buffer_size))
{
	...
	if ((fullPath_input[strnlen(fullPath_input, sizeof(fullPath_input)) - 1] != '\\') &&
		(fullPath_input[strnlen(fullPath_input, sizeof(fullPath_input)) - 1] != '/')) {
		strcat_s(fullPath_input, sizeof(fullPath_input), backslash);
	}
	/* Append wildcard character */
	snprintf(searchPath, sizeof(searchPath), "%s*", fullPath_input);
	...
	do {
		...
		if ((fullPath_output[strnlen(fullPath_output,
				sizeof(fullPath_output)) - 1] != '\\') &&
			(fullPath_output[strnlen(fullPath_output,
				sizeof(fullPath_output)) - 1] != '/')) {
