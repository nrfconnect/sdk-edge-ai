/*
 * Copyright (c) 2025 Nordic Semiconductor ASA
 *
 * SPDX-License-Identifier: LicenseRef-Nordic-5-Clause
 */

#include <stdio.h>
#include <stdarg.h>
#include <string.h>
#include <stdint.h>
#include <inttypes.h>
#include "axon/nrf_axon_platform.h"
#if (NOT_A_ZEPHYR_BUILD)
#include "axon/nrf_axon_platform_simulator.h"
#endif

#define axon_abs_u32(x) ((uint32_t)((int32_t)x < 0 ?  -1*(int32_t)x:x))

static void print_mismatch(unsigned int index, int difference,
	uint32_t margin, int32_t actual, int32_t expected)
{
	nrf_axon_platform_printf(
		"error @ %u: dif %d not less than %" PRIu32
		": got 0x%08" PRIx32 ", expected 0x%08" PRIx32 "\r\n",
		index, difference, margin, (uint32_t)actual, (uint32_t)expected);
}

int nrf_axon_verify_scalar_output_stride(const char *msg, const int32_t *output,
	int32_t expected_output, uint32_t count, uint32_t margin,
	uint8_t extra_output_stride)
{
	int err_cnt = 0;
	int error_dif;

	nrf_axon_platform_printf("Verify %s... ", msg);

	for (unsigned int i = 0; i < count; i++) {
		error_dif = output[i * (1+extra_output_stride)] - expected_output;
		if (axon_abs_u32(error_dif) > margin) {
			print_mismatch(i, error_dif, margin,
				output[i * (1 + extra_output_stride)], expected_output);
			err_cnt++;
		}
	}
	/* return error in case of mismatch */
	if (err_cnt > 0) {
		nrf_axon_platform_printf(" FAILED!: %d mismatches!\r\n", err_cnt);
		return -1;
	}
	nrf_axon_platform_printf(" PASSED!\r\n");
	return 0;
}

/*
 * Verifies output[] == expected_output[] with each value being within margin of each other.
 */
int nrf_axon_verify_vectors_output_stride(const char *msg, const int32_t *output,
	const int32_t *expected_output, uint32_t count, uint32_t margin,
	uint8_t extra_output_stride)
{
	int err_cnt = 0;
	int error_dif;

	nrf_axon_platform_printf("Verify %s... ", msg);

	for (unsigned int i = 0; i < count; i++) {
		error_dif = output[i * (1+extra_output_stride)] - expected_output[i];
		if (axon_abs_u32(error_dif) > margin) {
			print_mismatch(i, error_dif, margin,
				output[i * (1 + extra_output_stride)], expected_output[i]);
			err_cnt++;
		}
	}
	/* return error in case of mismatch */
	if (err_cnt > 0) {
		nrf_axon_platform_printf(" FAILED!: %d mismatches!\r\n", err_cnt);
		return -1;
	}
	nrf_axon_platform_printf(" PASSED!\r\n");
	return 0;

}

/*
 * Verifies output[] == expected_output[] with each value being within margin of each other.
 */
int nrf_axon_verify_vectors_expected_stride(const char *msg, const int32_t *output,
	const int32_t *expected_output, uint32_t count, uint32_t margin,
	uint8_t extra_expected_stride, uint8_t saturate_bits)
{
	int err_cnt = 0;
	int error_dif;
	int32_t saturate_min = -(1 << (saturate_bits - 1));
	int32_t saturate_max = -(saturate_min + 1);

	nrf_axon_platform_printf("Verify %s... ", msg);

	for (unsigned int i = 0; i < count; i++) {
		int32_t expected = expected_output[i * (1 + extra_expected_stride)];
		int32_t saturated_expected_output = expected < saturate_min ?
			saturate_min : expected > saturate_max ? saturate_max : expected;

		error_dif = output[i] - saturated_expected_output;
		if (axon_abs_u32(error_dif) > margin) {
			print_mismatch(i, error_dif, margin, output[i], saturated_expected_output);
			err_cnt++;
		}
	}
	/* return error in case of mismatch */
	if (err_cnt > 0) {
		nrf_axon_platform_printf(" FAILED!: %d mismatches!\r\n", err_cnt);
		return -1;
	}
	nrf_axon_platform_printf(" PASSED!\r\n");
	return 0;

}

int nrf_axon_verify_vectors(const char *msg, const int32_t *output,
	const int32_t *expected_output, uint32_t count, uint32_t margin,
	uint8_t extra_output_stride)
{
	return nrf_axon_verify_vectors_output_stride(msg, output, expected_output, count,
		margin, 0);
}


/*
 * verifies int16_t vectors
 */
int nrf_axon_verify_vectors_16(const char *msg, const int16_t *output,
	const int16_t *expected_output, uint32_t count, uint32_t margin)
{
	int err_cnt = 0;
	int error_dif;

	nrf_axon_platform_printf("Verify %s... ", msg);

	for (unsigned int i = 0; i < count; i++) {
		error_dif = output[i] - expected_output[i];
		if (axon_abs_u32(error_dif) > margin) {
			print_mismatch(i, error_dif, margin, output[i], expected_output[i]);
			err_cnt++;
		}
	}
	/* return error in case of mismatch */
	if (err_cnt > 0) {
		nrf_axon_platform_printf(" FAILED!: %d mismatches!\r\n", err_cnt);
		return -1;
	}
	nrf_axon_platform_printf(" PASSED!\r\n");
	return 0;

}

/*
 * verifies int8_t vectors
 */
int nrf_axon_verify_vectors_8(const char *msg, const int8_t *output,
	const int8_t *expected_output, uint32_t count, uint32_t margin)
{
	int err_cnt = 0;
	int error_dif;

	nrf_axon_platform_printf("Verify %s... ", msg);

	for (unsigned int i = 0; i < count; i++) {
		error_dif = output[i] - expected_output[i];
		if (axon_abs_u32(error_dif) > margin) {
			print_mismatch(i, error_dif, margin, output[i], expected_output[i]);
			err_cnt++;
		}
	}
	/* return error in case of mismatch */
	if (err_cnt > 0) {
		nrf_axon_platform_printf("FAILED!: %d mismatches!\r\n", err_cnt);
		return -1;
	}
	nrf_axon_platform_printf("PASSED!\r\n");
	return 0;

}
