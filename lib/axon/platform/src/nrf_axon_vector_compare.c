	...
	for (unsigned int i = 0; i < count; i++) {
		...
		if (axon_abs_u32(error_dif) > margin) {
			nrf_axon_platform_printf(
				" error @ %d: dif %d not less than %d: got 0x%.8lx, expected 0x%.8lx\r\n",
	...
	uint8_t extra_expected_stride, uint8_t saturate_bits)
{
  ...
  for (unsigned int i = 0; i < count; i++) {
    int32_t saturated_expected_output = expected_output[i * (1+extra_expected_stride)] < saturate_min ? saturate_min :
      expected_output[i * (1+extra_expected_stride)] > saturate_max ? saturate_max : expected_output[i * (1+extra_expected_stride)];
    error_dif = output[i] - saturated_expected_output;
    if (axon_abs_u32(error_dif) > margin) {
      nrf_axon_platform_printf(" error @ %d: dif %d not less than %d: got 0x%.8lx, expected 0x%.8lx\r\n", i, error_dif, margin, output[i], saturated_expected_output);
      err_cnt++;
    }
  }
  /* return error in case of mismatch */
