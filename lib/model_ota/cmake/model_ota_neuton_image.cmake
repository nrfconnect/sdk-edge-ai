...
#
#   1. Compiles a tiny generated stub that #includes MODEL_SRC with CONFIG_MODEL_OTA_NEUTON
#      *undefined* (so the compile-time `model_instance_` descriptor and its data arrays exist
#      and are referenced) and then #includes model_image_stub_body.h, which emits the partition
#      header (magic/version/size/crc + a DIRECT pointer to &model_instance_) into
#      section .model_image.header.
...
function(model_ota_neuton_image)
  ...
  # Generate the per-model stub. #undef of CONFIG_MODEL_OTA_NEUTON selects the compile-time
  # (const) model_instance_ path even though the app is built WITH that config; -imacros
  # autoconf.h only seeds the macro, so this #undef wins for the model.c include below.
  file(WRITE ${stub_c}
"/* Auto-generated by model_ota_neuton_image(); do not edit. */\n"
"#undef CONFIG_MODEL_OTA_NEUTON\n"
"#include \"${MI_MODEL_SRC}\"\n"
"#include \"${stub_body}\"\n")

  set(stub tgt_${MI_TARGET}_model_image_stub)
  add_library(${stub} OBJECT ${stub_c})
  target_link_libraries(${stub} PRIVATE zephyr_interface zephyr_generated_headers)
  ...
  # The stub regenerates the descriptor's compile-time pointers; make sure the model source
  # change re-triggers it.
  set_property(SOURCE ${stub_c} APPEND PROPERTY OBJECT_DEPENDS ${MI_MODEL_SRC} ${stub_body})
