...
static size_t model_image_partition_content_offset(const struct flash_area *fa)
{
#if IS_ENABLED(CONFIG_MCUBOOT_BOOTUTIL_LIB)
	uint32_t magic;
	int rc;
				  ...
				  size_t *content_offset_out)
{
	...
	content_offset = model_image_partition_content_offset(fa);
	...
	if (hdr.image_size < sizeof(hdr) ||
	    content_offset + hdr.image_size > fa->fa_size) {
		flash_area_close(fa);
	...
	const uint8_t *image_base = partition_addr + content_offset;
	...
	*hdr_out = hdr;
	if (content_offset_out != NULL) {
		*content_offset_out = content_offset;
	}
