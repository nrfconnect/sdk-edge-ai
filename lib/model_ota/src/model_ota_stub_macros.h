 ...
 *
 * Neuton partition image (model_ota_edgeai_neuton_model):
 ...
 *   MODEL_OTA_EDGEAI_SOLUTION_ID, MODEL_OTA_EDGEAI_NEUTON_MODEL_SRC,
 *   MODEL_OTA_PARTITION_NODELABEL, MODEL_OTA_NEUTON_NEURONS_CAP and MODEL_OTA_IMAGE_LINK_BASE;
 *   defines nrf_edgeai_load_user_model_<SOLUTION_ID>().
 *
 ...
 *   model_ota_axon_model_config.h and is compiled once per solution with -D defines
 *   (including MODEL_OTA_PARTITION_NODELABEL and MODEL_OTA_IMAGE_LINK_BASE); defines
 *   nrf_edgeai_load_user_model_<SOLUTION_ID>() (declared via model_ota_edgeai.h).
 *
 * Raw Axon app wired (model_ota_axon_model):
 *   lib/model_ota/src/model_ota_axon_wired.c includes the per-slot private
 *   model_ota_axon_model_config.h and is compiled once per TARGET with -D MODEL_OTA_AXON_TARGET,
 *   MODEL_OTA_PARTITION_NODELABEL and MODEL_OTA_IMAGE_LINK_BASE; defines
 ...
 *   solution flavours; MODEL_OTA_IMAGE_LINK_BASE - partition flash base from dt_reg_addr()
 *
 * Axon partition image stub (model_ota_axon_image_stub.c / model_ota_edgeai_axon_model):
 *   MODEL_OTA_IMAGE_LINK_BASE, MODEL_OTA_AXON_HEADER, optional MODEL_OTA_EDGEAI_AXON_MODEL_SRC
 *
 * Contract hash and image_size (probe, image stubs, wired loaders): MODEL_OTA_IMAGE_LINK_BASE,
 * supplied by CMake from the same dt_reg_addr() as the partition-image link --defsym.
 *
...
/** Cross-check @c MODEL_OTA_IMAGE_LINK_BASE against a devicetree mapped-partition nodelabel. */
#define MODEL_OTA_BUILD_ASSERT_IMAGE_LINK_BASE(label)                                              \
	BUILD_ASSERT((uintptr_t)MODEL_OTA_PARTITION_ADDR(label) ==                                 \
			     (uintptr_t)MODEL_OTA_IMAGE_LINK_BASE,                                 \
		     STRINGIFY(label) " address must match MODEL_OTA_IMAGE_LINK_BASE")
