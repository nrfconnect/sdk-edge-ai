...
The nRF Machine Learning application gathers data from sensors, forwards data to the Edge Impulse platform, and runs the selected machine learning model.
...
* nRF54LM20 DK - This development kit does not have a built-in accelerometer.
  However, this application is used with an accelerometer (``ADXL362``) on the PCA63566 shield.
  It is connected to the DK with jumper wires.
...
* The nRF54LM20 DK uses :ref:`zephyr:uart_api`.

Machine learning model
======================

The application handles the machine learning model using the selected model runner as described in :ref:`nrf_machine_learning_app_model_runners`.
...
* The nRF54LM20 DK uses the hardware accelerometer machine learning model located in the :file:`applications/machine_learning/models` directory.
  The model uses the data from the built-in accelerometer to recognize the following gestures:

  * ``circle`` - The device is moved in a circle.
  * ``hold`` - The device is held steadily.
  * ``tap`` - The device is tapped.
  * ``tilt`` - The device is tilted like single nod.
  * ``updown`` - The device is moved in up-down direction.

  Anomalies are not recognized by this model.
...
You can also use the `Neuton platform_` to train and deploy tiny custom machine learning models.
...
Most of the available configurations use only built-in sensors or the simulated sensor signal.
You do not need to connect any additional components to the board.
Only for the nRF54LM20 DK you need to connect the accelerometer with jumper wires.
...
.. tabs::
   ...
   .. group-tab:: nRF54LM20 DK
      ...
      **Button 0** switches between data forwarding and running the machine learning model.
...
.. tabs::
   ...
   .. group-tab:: nRF54LM20 DK
      ...
      * If the device is returning the machine learning prediction results, a pattern of LEDs are lit.
        The machine learning result is represented by the LEDs as follows:

         * **LED0** - ``circle``
         * **LED1** - ``hold``
         * **LED2** - ``tap``
         * **LED0** and **LED2** - ``tilt``
         * **LED1** and **LED2** - ``updown``

        If the machine learning model is not running or it has not detected anything yet, **LED0**, **LED1**, and **LED2** keep blinking.
      * If the device forwards data, **LED0**, **LED1** and **LED2** have the following blinking patterns:
...
.. list-table:: nRF Machine Learning build types
   ...
   * - Debug Neuton
     - :file:`prj_neuton.conf`
     - ``nrf54lm20dk/nrf54lm20a/cpuapp``
     - Similar to Debug, but uses Neuton model.
   * - Profiling
     - :file:`prj_profile_*.conf`
     - ``nrf54lm20dk/nrf54lm20a/cpuapp``
     - Version of application used for profiling, see :ref:`nrf_machine_learning_app_profiling`.
...
1. Turn on your DK.
   The application starts in a mode that runs the machine learning model.
   **LED0**, **LED1**, and **LED2** are blinking because no gesture has been recognized by the machine learning model yet.
#. Move the device in circle.
   The ``circle`` gesture is recognized by the machine learning model.
   **LED0** turns on.
#. Hold the device steady.
   The ``hold`` gesture is recognized by the machine learning model.
   **LED1** turns on.
#. Tap the device.
   The ``tap`` gesture is recognized by the machine learning model.
   **LED2** turns on.
#. Tilt the device few times.
   The ``tilt`` gesture is recognized by the machine learning model.
   **LED0** and **LED2** turn on.
#. Move the device up and down.
   The ``updown`` gesture is recognized by the machine learning model.
...
#. Connect to the device from the application.
   The device is advertising as ``Sensor HUB``.
   The services of the connected device are shown.
   If the device cannot be found, the DK might be connected to another device.
   Terminate the connection with the other device, refresh the scanning, and connect to the DK.
   If the device disconnects shortly after being connected, the DK might have saved bonding data from previous connection.
   To erase the bonding data, double-click the **Button 0** within three seconds after the application reboots.
#. Find **Nordic Status Message Service** by its name or UUID (``57a70001-9350-11ed-a1eb-0242ac120002``).
#. Read its **Characteristic User Description** to see that it is ``Gesture``.
#. Read the **Nordic Status Message Service** message characteristic to check the initial status.
   It presents the status string that describes the previously detected gesture.
#. Enable notification for the characteristic.
#. Repeat **steps 2 to 6** from list above.
   **Nordic Status Message Service** is updated after gesture change.
#. Disconnect the `nRF Connect for Mobile`_ application from the device.
...
1. Press and hold **Button 0** for more than five seconds to switch to the data forwarding mode.
   After the mode is switched, **LED0**, **LED1**, and **LED2** start blinking very slowly.
#. Connect to the DK with a terminal emulator (for example, the `Serial Terminal app`_).
   See :ref:`test_and_optimize` for the required settings.
#. Observe the sensor readouts represented as comma-separated values.
   Every line represents a single sensor readout.
#. Turn off the terminal emulator to ensure that only one program has access to the data on UART.
