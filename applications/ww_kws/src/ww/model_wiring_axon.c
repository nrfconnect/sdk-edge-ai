...
/*
 * Non-generated counterpart of nrf_edgeai_generated/nrf_edgeai_user_model.c: everything about
 * the input pipeline, DSP feature extraction, and interfaces below is copied verbatim from that
 * generated file (it is part of the application, not the updatable model package), but the
 * model instance itself is wired up differently depending on CONFIG_APP_MODEL_OTA - see
 * ww_model_ota_load() at the bottom. Kept as its own file (rather than patching the generated
 * one) so a re-generated model still drops in cleanly; keep the two in sync if the model's
 * feature-extraction/DSP configuration ever changes.
 */

#include "model_wiring.h"

#include "nrf_edgeai_generated/nrf_edgeai_user_types.h"
 ...
 * once a valid package has been read from the model_storage_ww flash partition. persistent_vars
 * is the RAM backing store the model's cmd_buffer references directly by address (see
 * doc/libraries/model_ota.rst) - it must exist somewhere in the application regardless of
 * whether a model is currently loaded, sized to match the generated header's own
 * axon_model_axon_user_instance_36711_persistent_vars[] (see
 * nrf_edgeai_generated/nrf_edgeai_user_model_axon.h) via CONFIG_APP_WW_PERSISTENT_VARS_SIZE.
 */
static nrf_axon_nn_compiled_model_s model_instance_;
int32_t axon_model_axon_user_instance_36711_persistent_vars[CONFIG_APP_WW_PERSISTENT_VARS_SIZE];
#define P_MODEL_INSTANCE &model_instance_
#else  // !CONFIG_APP_MODEL_OTA
 ...
 */
#include "nrf_edgeai_generated/nrf_edgeai_user_model_axon.h"
...
LOG_MODULE_REGISTER(ww_model_wiring, LOG_LEVEL_INF);

/*
 * Fail the build with a clear message if this board's devicetree overlay doesn't define the
 * model_storage_ww partition node model_ota loads from, instead of a much less obvious error
 * deep inside a flash_map.h macro expansion.
 */
...
nrf_edgeai_t *ww_model_ota_load(void)
{
	...
	err = model_pkg_load_axon(PARTITION_ID(model_storage_ww),
				   (const uint8_t *)PARTITION_ADDRESS(model_storage_ww),
				   &model_instance_, &info);
	if (err != MODEL_PKG_OK) {
		LOG_ERR("No usable WW model in model_storage_ww (err %d)", err);
		return NULL;
	}

	LOG_INF("Active WW model: '%s' version 0x%08x (%u cmd words, %u B const)", info.name,
