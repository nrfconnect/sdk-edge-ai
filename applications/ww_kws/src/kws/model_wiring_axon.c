 ...
 * kws_model_ota_load() at the bottom. Kept as its own file (rather than patching the generated
 ...
 * once a valid package has been read from the model_storage_kws flash partition.
 * persistent_vars is the RAM backing store the model's cmd_buffer references directly by
 * address (see doc/libraries/model_ota.rst) - it must exist somewhere in the application
 * regardless of whether a model is currently loaded, sized to match the generated header's own
 * axon_model_axon_user_instance_36712_persistent_vars[] (see
 * nrf_edgeai_generated/nrf_edgeai_user_model_axon.h) via CONFIG_APP_KWS_PERSISTENT_VARS_SIZE.
 */
...
LOG_MODULE_REGISTER(kws_model_wiring, LOG_LEVEL_INF);

/*
 * Fail the build with a clear message if this board's devicetree overlay doesn't define the
 * model_storage_kws partition node model_ota loads from, instead of a much less obvious error
...
nrf_edgeai_t *kws_model_ota_load(void)
{
	...
	err = model_pkg_load_axon(PARTITION_ID(model_storage_kws),
				   (const uint8_t *)PARTITION_ADDRESS(model_storage_kws),
				   &model_instance_, &info);
	if (err != MODEL_PKG_OK) {
		LOG_ERR("No usable KWS model in model_storage_kws (err %d)", err);
		return NULL;
	}

	LOG_INF("Active KWS model: '%s' version 0x%08x (%u cmd words, %u B const)", info.name,
