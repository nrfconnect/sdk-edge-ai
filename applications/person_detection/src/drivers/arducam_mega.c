...
LOG_MODULE_REGISTER(DT_DRV_COMPAT, LOG_LEVEL_WRN);
...
static int arducam_mega_check_connection(const struct device *dev)
{
	...
	if (!(cam_id & 0x87)) {
		LOG_ERR("arducam mega not detected, 0x%x", cam_id);
		return -ENODEV;
	}

	LOG_INF("detect camera id 0x%x, ret = %d", cam_id, ret);
...
static int arducam_mega_stream_start(const struct device *dev)
{
	...
	k_timer_start(&drv_data->stream_schedule_timer, K_NO_WAIT, K_MSEC(30));
...
static int arducam_mega_capture(const struct device *dev, uint32_t *length)
{
	...
	do {
		...
		k_msleep(2);
		/* Workaround for stopping stream during capture */
		if (!drv_data->stream_on) {
			return -EALREADY;
		}
