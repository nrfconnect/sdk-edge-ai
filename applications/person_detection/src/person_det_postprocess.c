	...
	for (unsigned y = 0; y < fh; y++) {
		for (unsigned x = 0; x < fw; x++) {
			for (int a = 0; a < NUM_ANCHORS; a++) {
				...
				float rcx = dequant(read_ch(base, fh, row_stride, (uint16_t)(o + 0), y, x),
						    dqm, dqr, dqz);
				float rcy = dequant(read_ch(base, fh, row_stride, (uint16_t)(o + 1), y, x),
						    dqm, dqr, dqz);
				float rw = dequant(read_ch(base, fh, row_stride, (uint16_t)(o + 2), y, x),
						   dqm, dqr, dqz);
				float rh = dequant(read_ch(base, fh, row_stride, (uint16_t)(o + 3), y, x),
						   dqm, dqr, dqz);
				float robj = dequant(read_ch(base, fh, row_stride, (uint16_t)(o + 4), y, x),
						     dqm, dqr, dqz);
				float rcls = dequant(read_ch(base, fh, row_stride, (uint16_t)(o + 5), y, x),
						     dqm, dqr, dqz);

				float cx = (sigmoid(rcx) + (float)x) * (float)stride_px;
				float cy = (sigmoid(rcy) + (float)y) * (float)stride_px;
				...
				float conf = sigmoid(robj) * sigmoid(rcls);
