...
#define M_S2_TO_G  9.80665f  /* standard gravity in m/s^2 */

static inline float convert_m_s2_to_g(float m_s2)
{
	return m_s2 / M_S2_TO_G;
}
...
int imu_init(const imu_config_t *p_config,
		    generic_cb_t data_ready_cb)
{
	int res = 0;
	...
	HW_RETURN_IF(imu_ctx.dev == NULL, -ENODEV);
	HW_RETURN_IF(!device_is_ready(imu_ctx.dev), -ENODEV);

	if (p_config->accel_enabled) {
...
int imu_read(imu_data_t *const p_data)
{
	...
	for (i = 0; i < IMU_NUM_AXES; i++) {
		p_data->accel[i].m_s2 = (float)sensor_value_to_double(&acc[i]);
		p_data->accel[i].g = convert_m_s2_to_g(p_data->accel[i].m_s2);

		p_data->gyro[i].rad_s = (float)sensor_value_to_double(&gyr[i]);
	}
