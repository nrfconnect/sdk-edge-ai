 ...
 *
 * nrf_axon_platform_disable_interrupts() returns the interrupt state immediately prior to
 ...
 *
 * Non-blocking call to reserve the axons hardware for the driver to service the asynchronous
 * command queue.
 * The driver will never call this function more than once before calling
 * nrf_axon_platform_reserve_for_driver().
 * The driver will always call nrf_axon_platform_reserve_for_driver() exactly once after calling
 * this function.
 *
 * @note
 ...
 * must check with the driver to see if it is waiting for the reservation
 * (via nrf_axon_queue_not_empty())
 ...
 * This function must not be called from user code!!!
 *
 * This function is called by the driver on behalf of the user when a synchronous inference is
 * started.
 * Users can call this function in advance if they want to access the interlayer buffer directly,
 * before inference is invoked.
 * Calling this function after inference has completed does not guarantee that the interlayer buffer
 * contents are unchanged.
 *
 * This is a blocking function.
 * This function can get called multiple times before nrf_axon_platform_free_reservation_from_user()
 * is called.
 *
 ...
 *
 * In an RTOS system, the platform code signals the driver's thread (also created by the
 * platform), which in-turn invokes nrf_axon_driver_process_event().
 *
 * In a bare-metal system, nrf_axon_driver_process_event() can be called directly from
 ...
 *
 * In synchronous mode, driver will invoke nrf_axon_platform_wait_for_user_event() on behalf of the
 * user to wait for the operationto complete from the user's execution context.
 *
 * nrf_axon_platform_generate_user_event() will be invoked by the driver (in the drivers execution
 * context)
 * from nrf_axon_process_driver_event() when the user's job is complete. This function must generate
 * the event that nrf_axon_platform_wait_for_user_event is waiting on.
 *
 * In an RTOS system, this pair of functions should be implemented with a semaphore with maximum
 * count of 1 and default 0 (available).
