 ...
 * @brief Macro to declare feature scaling interface functions for various data types
 * and scaling modes.
 *
 * All declared functions have the following prototype:
 * @code
 * nrf_edgeai_err_t nrf_edgeai_scale_features_<mode>_<input_type>_<output_type>(nrf_edgeai_input_t*
 *      p_input, nrf_edgeai_dsp_pipeline_t* p_dsp);
 * @endcode
 * where:
...
#define NRF_EDGEAI_DECLARE_SCALE_FEATURES_INTERFACE(interface_name)  \
	nrf_edgeai_err_t nrf_edgeai_scale_features_##interface_name( \
		nrf_edgeai_input_t *p_input, nrf_edgeai_dsp_pipeline_t *p_dsp)
 ...
 *
 * @note Example functions: nrf_edgeai_scale_features_input_vector_i8_q8,
 * nrf_edgeai_scale_features_input_vector_f32_q16, etc.
 */
 ...
 * Scaling is performed using per-feature min/max values and supports selective scaling based on
 * feature masks.
 *
 * @param[in, out] p_input        Pointer to the input processing context @ref nrf_edgeai_input_t
 * @param[in, out] p_dsp          Pointer to the DSP pipeline context @ref nrf_edgeai_dsp_pipeline_t
 * @return Status code indicating success or error.
 *
 * @note Example functions: nrf_edgeai_scale_features_input_window_i8_q8,
 * nrf_edgeai_scale_features_input_window_f32_f32, etc.
 */
 ...
 *
 * These functions perform scaling of digital signal processing (DSP) features (such as statistical
 * or spectral features) from the input data. The scaling is performed according to the
 * configuration in the neural network context.
 *
 * @param[in, out] p_input        Pointer to the input processing context @ref nrf_edgeai_input_t
 * @param[in, out] p_dsp          Pointer to the DSP pipeline context @ref nrf_edgeai_dsp_pipeline_t
 * @return Status code indicating success or error.
 *
 * @note Example functions: nrf_edgeai_scale_features_dsp_i8_q8,
 * nrf_edgeai_scale_features_dsp_f32_f32, etc.
 */
