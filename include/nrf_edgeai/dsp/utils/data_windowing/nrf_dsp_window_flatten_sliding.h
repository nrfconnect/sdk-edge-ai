 ...
 * @param[in]    max_samples_num     Maximum number of feature samples in window buffer, feature
 *                                   sample = 1 * uniq_features_num
 * @param[in]    uniq_features_num   Number of unique features in feature sample,
 *                                   e.g feature sample = {x, y, z}, then uniq_features_num = 3
 * @param[in]    window_shift        Window shift in samples by which the window will be shifted
 * when it is filled. Must be less or equal @p max_samples_num
 *
 ...
 *        with ordering by feature columns, e.g p_input = [x,y,z,x1,y1,z1] ->
 *        p_window = [x,x1,..,y,y1,.., z,z1,..]
 *
 * @note If window was filled, next call will shift window to the right for each unique feature in
 * window by p_ctx->window_shift
 *
 * @param[in, out]  p_ctx       Pointer to the window context, should be initialized first @ref
 * nrf_dsp_window_flatsliding_init
 * @param[in]       p_input     Pointer to the feature sample buffer
 * @param[in]       num         Number of samples in the input buffer, num = buffer_size /
 * p_ctx->uniq_features_num
 *
 ...
 *          p_window = [x,x1,..,y,y1,.., z,z1,..]
 *
 * @param[in, out]  p_ctx       Pointer to the window context,
 *                              should be initialized first @ref nrf_dsp_window_flatsliding_init
 * @param[in]       p_input     Pointer to the feature sample buffer
 * @param[in]       num         Number of samples in the input buffer,
 *                              num = buffer_size / p_ctx->uniq_features_num
 *
 * @return uint16_t Number of remaining samples to collect window
 */
...
/**
 * @brief Feeds f32 samples to the floating-point flatten by colums window,
 *        with ordering by feature columns, e.g p_input = [x,y,z,x1,y1,z1] -> p_window = [x,x1,..,
 * y,y1,.., z,z1,..], usage of inputs column is determined by p_input_usage_mask where each bit
 * represents each unique feature
 *
 * @param[in, out]  p_ctx           Pointer to the window context,
 *                                  should be initialized first @ref nrf_dsp_window_flatsliding_init
 * @param[in]       p_input         Pointer to the feature sample buffer
 ...
 *          with ordering by feature columns, e.g p_input = [x,y,z,x1,y1,z1] ->
 *          p_window = [x,x1,.., y,y1,.., z,z1,..], usage of inputs column is determined by
 *          p_input_usage_mask where each bit represents each unique feature
 *
 * @param[in, out]  p_ctx           Pointer to the window context,
 *                                  should be initialized first @ref nrf_dsp_window_flatsliding_init
 * @param[in]       p_input         Pointer to the feature sample buffer
