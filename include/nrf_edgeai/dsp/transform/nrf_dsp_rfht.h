...
/**
 *
 * @defgroup nrf_dsp_transform_rfht Fast Hartley Transform Real
 * @{
 ...
 *
 */
#ifndef _NRF_DSP_TRANSFORM_REAL_FHT_H_
#define _NRF_DSP_TRANSFORM_REAL_FHT_H_
 ...
 */
typedef struct nrf_dsp_rfht_instance_s {
	...
	void *p_window;
	/** Window buffer size to store result of FHT, should be powers of two in value
	 * (128/256/512/etc)
	 */
	uint16_t window_size;
	/** Cached value of log2(window_size) */
	uint16_t log_n;
} nrf_dsp_rfht_instance_t;
 ...
 * @param[in]     p_sin_table           Pointer to sin() table, with size of window_size / 4
 * @param[in]     p_tg_table            Pointer to tg() table, with size of window_size / 4
 * @param[in]     p_rev_bit_index       Pointer to index reordering array, with size of window_size
 * @param[in]     p_window              Pointer to RFHT result buffer, with size of window_size
 * @param[in]     window_size           Window buffer size to store result of FHT, should be powers
 *                                      of two in value (128/256/512/etc)
 ...
 * @param[in] p_inst     Pointer to the RFHT instance
 *
 * @return Number of fraction bit for the fractional part of the value in f32 format
 */
...
#endif /* _NRF_DSP_TRANSFORM_REAL_FHT_H_ */
