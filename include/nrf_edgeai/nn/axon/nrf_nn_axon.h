 ...
 *
 * @note Called internally by nrf_nn_axon_init, but can be called separately if persistent vars need
 * to be re-initialized without re-initializing the whole model (e.g. between streaming sessions).
 *
 * @param[in] p_nn Pointer to Axon NN model.
 *
 * @return Operational status code @ref nrf_axon_result_e
