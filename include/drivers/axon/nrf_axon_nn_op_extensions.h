 ...
 * By organizing this as a series of unions, the upper 32bits of the parameters remain 0s unless the arguement is a pointer.
 * 
 * Op Extension Base 2 profile arguments. The following conditions must be met for an op extension to use
 * this structure to pass its parameters. (If the conditions aren't met, a new structure needs to be defined.)
 * - A single input vector.
...
typedef struct  {
  ...
  struct {
    union {
      NRF_AXON_PLATFORM_BITWIDTH_UNSIGNED_TYPE spacer0;
      struct {
        uint16_t input_height;          /*< height in elements of the input */
        uint16_t input_width;           /*< width in elements of the input */
      };   
    }; 
    union {
      NRF_AXON_PLATFORM_BITWIDTH_UNSIGNED_TYPE spacer1;
      struct {
        uint16_t output_height;          /*< height in elements of the output */
        uint16_t output_width;           /*< width in elements of the output */
      };
    };
    union {
      NRF_AXON_PLATFORM_BITWIDTH_UNSIGNED_TYPE spacer2;
      struct {
        uint16_t input_channel_cnt;           /*< channel count in elements of the input */
        uint16_t output_channel_cnt;           /*< channel count in elements of the output */
      };   
    }; 
    union {
      NRF_AXON_PLATFORM_BITWIDTH_UNSIGNED_TYPE spacer3;
      struct {
        uint16_t input_stride;       /*< If true, input rows begin on the following byte from the previous row. If false, rows are aligned to 4byte boundaries. */
      };
    };
  } remaining_args;
...
/**
 * @brief Structure compiled into the command buffer then passed to the CPU op extension function resize_nearest_neighbor
 * 
 * @note To maintain compatibilty with simulator builds, the parameters are in elements of NRF_AXON_PLATFORM_BITWIDTH_UNSIGNED_TYPE.
 * This is 64bits on the simulator, 32bits on Nordic MCUs.
 * By organizing this as a series of unions, the upper 32bits of the parameters remain 0s unless the arguement is a pointer.
 ...
 * 
 * @param[in] argc number of elements in args. Must equal sizeof(nrf_axon_nn_op_extension_resize_nearest_neighbor_args_s)/sizeof(NRF_AXON_PLATFORM_BITWIDTH_UNSIGNED_TYPE)
 * @param[in] args up-casted nrf_axon_nn_op_extension_resize_nearest_neighbor_args_s, with parameters to the function.
 */
