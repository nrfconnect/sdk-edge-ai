 ...
 */
typedef struct {
	/**< base compiled model information. */
 ...
 */
typedef struct {
	/**< name that gets printed with the test. */
	const char *test_name;
	/**< 1 or more fully model input test vectors */
	const int8_t **full_model_input_vectors;
	/**< full model expected output vectors, one for each full model input vector */
	const int8_t **full_model_expected_output_vectors;
	/**< number of full model test/expected_output vector pairs. */
	uint16_t full_model_vector_count;
	/**< individual layer vectors. These serve as both input and expected output. */
	const int8_t **layer_vectors;
	/**< number of elements in layer_models and layer_vectors */
	uint16_t layer_cnt;
 ...
 * @param[in] test_group_name Name of the collective set of tests. Printed at the start and end of
 *            the test.
 * @param[in] models_count Number of elements in *compiled_full_models.
 * @param[in] compiled_1_layer_models Optional. If not null, the list of layer models for each
 *            corresponding model in compiled_full_models.
 ...
 */
int nrf_axon_nn_run_test_vectors(
	const nrf_axon_nn_compiled_model_s **compiled_full_models,
 ...
 */
int nrf_axon_print_test_inference_results(
	const nrf_axon_nn_compiled_model_s *compiled_model,
	int8_t *output_ptr,
	uint32_t profiling_ticks);

/**
 * @brief Copies a padded layer model output to an unpadded buffer
 *
 * Used internally. Layer models can have 4 dimensional output
 * (batches)
 */
