...
#define MODEL_PKG_FORMAT_VERSION 9
...
/**
 * Precision of a Neuton package's MODEL_PKG_NEUTON_SEC_WEIGHTS/ACT_WEIGHTS sections, matching
 * MODEL_PARAMS_TYPE in the generated model source and selecting which union member of @ref
 * nrf_edgeai_model_neuton_params_u the loader populates. Every other section (links/indices/
 * act_type_mask, and the output-scale floats) is precision-independent.
 */
enum model_pkg_neuton_params_type {
	MODEL_PKG_NEUTON_PARAMS_F32 = 0, /**< weights: float, act_weights: float */
	MODEL_PKG_NEUTON_PARAMS_Q16 = 1, /**< weights: int16_t, act_weights: uint16_t */
	MODEL_PKG_NEUTON_PARAMS_Q8 = 2,  /**< weights: int8_t, act_weights: uint8_t */
};
 ...
 *
 * Each section is placed at the next 4-byte-aligned offset from the start of the payload
 * (which itself starts 4-byte aligned, since @ref model_pkg_header's own size is a multiple
 * of 4 and model_storage partitions are flash-page aligned): the packer
 * (tools/model_ota/package_model.py's build_package()) inserts zero-padding bytes between
 * sections as needed, and the loader (model_pkg_load_neuton()) skips the same padding when
 * locating each section, so section_len never includes it. This matters for q16/q8 packages,
 * where MODEL_PKG_NEUTON_SEC_WEIGHTS/ACT_WEIGHTS's 1-/2-byte elements can otherwise leave
 * whatever follows them less than 4-byte aligned - which faults on real Cortex-M hardware
 * once the Neuton runtime's q16/q8 inference kernels read that data with word-wide (SIMD)
 * loads, even though plain unaligned loads/stores are not trapped by default on those same
 * targets. MODEL_PKG_NEUTON_SEC_ACT_TYPE_MASK in particular can have an odd byte length
 * (ceil(neurons_num/8)), so it must stay last regardless of precision.
 */
enum model_pkg_neuton_section {
	MODEL_PKG_NEUTON_SEC_WEIGHTS = 0,        /**< weights_num elements, see @ref model_pkg_neuton_params_type */
	MODEL_PKG_NEUTON_SEC_ACT_WEIGHTS,        /**< neurons_num elements, see @ref model_pkg_neuton_params_type */
	MODEL_PKG_NEUTON_SEC_OUTPUT_SCALE_MIN,   /**< float[outputs_num]; empty for classification (no scale concept) */
	MODEL_PKG_NEUTON_SEC_OUTPUT_SCALE_MAX,   /**< float[outputs_num]; empty for classification (no scale concept) */
	MODEL_PKG_NEUTON_SEC_AVERAGE_EMBEDDING,  /**< float[outputs_num]; empty except for anomaly detection */
 ...
 * model_type, payload_size vs. buffer capacity, section_len[] (plus inter-section alignment
 * padding, see @ref model_pkg_neuton_section) vs. payload_size, then CRC32 over the whole
 * header (with crc32 itself treated as 0) plus the payload.
 */
struct model_pkg_header {
	uint8_t magic[4]; /**< {'N','E','A','I'} */
	uint16_t format_version;
	uint8_t model_type; /**< enum model_pkg_type */
	uint8_t params_type; /**< enum model_pkg_neuton_params_type; unused (0) for Axon packages */
	char name[MODEL_PKG_NAME_LEN];
	uint32_t model_version; /**< free-form, e.g. major.minor.patch packed by the host tool */
	/** Bytes following this header, i.e. sum(section_len) plus the zero-padding bytes
	 *  inserted between sections to keep each one 4-byte aligned - see @ref
	 *  model_pkg_neuton_section.
	 */
...
struct model_pkg_neuton_info {
	...
	const float *output_scale_min;   /**< output_scale_min[outputs_num]; empty section if unused (classification) */
	const float *output_scale_max;   /**< output_scale_max[outputs_num]; empty section if unused (classification) */
	const float *average_embedding;  /**< average_embedding[outputs_num]; empty section unless anomaly detection */
};
...
enum model_pkg_result {
	...
	MODEL_PKG_ERR_BAD_PARAMS_TYPE = -15,
};

/**
 * @brief Load, validate, and wire up a Neuton model package directly from a memory-mapped
 * flash partition (XIP; no RAM copy of the payload itself).
 *
 * On success, out_model's meta/params array pointers point directly into partition_addr (i.e.
 * into flash), except p_neurons which points at the caller-owned neurons_buf; which union
 * member of out_model->params was populated (f32/q16/q8) depends on the package's own
 * precision (see @ref model_pkg_neuton_params_type), not on anything the caller passes in.
 * out_model is left untouched on failure.
 *
 * @param[in]  fa_id           Flash area ID of the partition to load from, e.g.
 *                              FIXED_PARTITION_ID(model_partition). Mirrors @ref
 *                              model_pkg_load_axon's fa_id/partition_addr pair, so that an
 *                              application hosting more than one independently updatable
 *                              Neuton model can call this once per model, against each one's
 *                              own partition - see @ref model_pkg_load_axon's own doc comment
 *                              for why both are needed rather than just one.
 ...
 *                              (out_model's p_neurons will point here). Must point at an array
 *                              of the package's own element type (float/int16_t/uint8_t for
 *                              f32/q16/q8 respectively) - the caller is expected to know this
 *                              from the model it provisions, the same way it already knows
 *                              which nrf_edgeai_user_model_*() accessor to call.
