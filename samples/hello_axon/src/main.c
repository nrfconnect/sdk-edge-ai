		       ...
		       const nrf_axon_nn_compiled_model_s *model)
{
	...
	const uint8_t deq_zp = model->output_dequant_zp;
...
static void sync_flow(void)
{
...
#if IS_ENABLED(CONFIG_AVOID_INPUT_DOUBLE_BUFFER)
	...
	for (size_t i = 0; i < ARRAY_SIZE(sample_values); i++) {
		...
		quantize(&sample_value, 1, input_buffer,
		...
		dequantize(output, 1, &prediction, &model_hello_axon);
