...
The Data forwarder sample streams real sensor measurements from a development kit to a host computer.
...
Depending on the selected transport and Bluetooth® Low Energy availability on the host, a second development kit running the `Nordic central UART sample`_ as a USB serial bridge may be required when the host cannot connect to the peripheral over Bluetooth LE directly.
...
UART transport
   ...
   .. note::
      UART transport is not available on nRF54L15 TAG without any external UART-to-USB converter.

Increasing throughput
---------------------

You can improve streaming throughput with the following adjustments.
...
Bluetooth LE with direct host connection
   ...
   #. Follow the Bluetooth LE throughput procedure in the host tool documentation for additional OS-specific tuning steps.
...
#. Add a new ``config DATA_FWD_SENSOR_*`` entry to the ``DATA_FWD_SENSOR`` choice in :file:`Kconfig`.
...
.. tabs::
   ...
   .. group-tab:: Edge Impulse CLI
      ...
      The sample can output comma-separated sensor values compatible with the ``edge-impulse-data-forwarder`` CLI tool when ``CONFIG_DATA_FWD_PROTO_ASCII_MODE`` Kconfig option is enabled.
      This replaces CBOR binary framing with CSV lines (one sample per line, values separated by commas, terminated with a CRLF line ending) compatible with protocol specified by `Edge Impulse's data forwarder`_.
      ...
      #. Use a second development kit running the `Nordic central UART sample_` sample as a serial bridge.
...
.. tabs::
   ...
   .. group-tab:: Data forwarder host tool
      ...
      #. Run :ref:`data_forwarder_host_tool` on the host computer and connect to the device over Bluetooth LE.
         ...
         Alternatively, if the host has no suitable Bluetooth LE adapter, use a second development kit running the `Nordic central UART sample_` as a serial bridge: flash Central UART on the second kit, wait for it to connect to the peripheral, then connect the host tool to the Central UART USB serial port.

      #. Enter the label and start recording of sampling session.
         For details refer to :ref:`data_forwarder_host_tool` documentation.
   ...
   .. group-tab:: Edge Impulse CLI
      ...
      #. Identify data interface.
         With the default build, the data stream is forwarded over Bluetooth LE NUS (via Central UART) or a dedicated data UART when UART transport is enabled.
         Do not open the data UART or Central UART data port in a terminal application while ``edge-impulse-data-forwarder`` is running.
