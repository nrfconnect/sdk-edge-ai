 ...
 */
static flt32_t fill_features_buffer(flt32_t*     p_buffer,
 ...
 *   regression task.
 */
static flt32_t model_predict(nrf_edgeai_t* p_user_model,
                             flt32_t*      p_input_features,
                             size_t        features_num)
{
    ...
    if (res == NRF_EDGEAI_ERR_SUCCESS)
    {
        ...
        if (res == NRF_EDGEAI_ERR_SUCCESS)
        {
            ...
            assert(p_user_model->decoded_output.regression.outputs_num == USER_MODELS_OUTPUTS_NUM);
...
int main(void)
{
    ...
    for (size_t i = 0; i < sizeof(USER_INPUT_DATA) / sizeof(USER_INPUT_DATA[0]); i++)
    {
        ...
        flt32_t ground_truth = fill_features_buffer(input_features, USER_UNIQ_INPUTS_NUM, i);

        /* Run neural network inference with the extracted features */
        flt32_t predicted_value = model_predict(p_user_model, input_features, USER_UNIQ_INPUTS_NUM);

        /* Calculate absolute error: magnitude of difference between prediction and ground truth */
        flt32_t abs_err = fabsf(predicted_value - ground_truth);

        /* Display results for this test sample */
        printk("Air quality - Predicted value: %f, Expected value: %f, absolute error %f\r\n",
               predicted_value,
               ground_truth,
               abs_err);
    }
