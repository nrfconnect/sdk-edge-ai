...
BUILD_ASSERT(FIXED_PARTITION_EXISTS(model_storage),
	     "board devicetree is missing model_storage - see boards/*.overlay");
#endif
...
int main(void)
{
	...
	nrf_edgeai_t *p_user_model = nrf_edgeai_load_user_model_36025(
		PARTITION_ID(model_storage),
		(const uint8_t *)PARTITION_ADDRESS(model_storage));
#elif defined(CONFIG_NRF_EDGEAI_REGRESSION_MODEL_NEUTON)
