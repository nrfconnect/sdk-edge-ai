...
static bool model_ota_confirm_pending(void)
{
	int swap_type = mcuboot_swap_type_multi(MODEL_IMAGE_INDEX);

	if (swap_type < 0) {
		LOG_ERR("Failed to read model image swap type: %d", swap_type);
		return false;
	}

	return swap_type == BOOT_SWAP_TYPE_REVERT;
}

static void try_confirm_model_ota(void)
{
	int rc;

	if (model_ota_confirm_attempted || !model_ota_confirm_pending()) {
		return;
	}

	model_ota_confirm_attempted = true;

	rc = boot_write_img_confirmed_multi(MODEL_IMAGE_INDEX);
	if (rc != 0) {
		LOG_ERR("Model loaded OK but MCUboot confirm failed: %d", rc);
		return;
	}
