...
Confirms, after linking:
  ...
  - the image was linked at the expected flash address (__model_image_start == image link addr),
  - the header sits first, at the link address,
...
def main(argv=None):
    ...
    parser.add_argument("--partition-size", type=lambda x: int(x, 0),
                        help="Full partition size from devicetree")
    parser.add_argument("--model-image-offset", type=lambda x: int(x, 0), default=0,
