...
from elf_const import ELFFile, SymbolTableSection, require_pyelftools
from model_contract import symbol_name_hash
...
def compiled_model_size_from_probe(probe: Path) -> int:
    if not probe.is_file():
        raise ValueError(f"probe object not found: {probe}")
    markers = load_symbol_index(probe).defined(MODEL_SIZE_MARKER)
    if len(markers) != 1:
        raise ValueError(f"probe must define exactly one {MODEL_SIZE_MARKER} marker")
...
def collect_provide_symbols(objects: Sequence[Path]) -> list[str]:
    ...
    for obj in objects:
        index = load_symbol_index(obj)
        for entry in index.symbols:
            if not entry.is_global or not entry.name:
                continue
            if entry.is_undefined:
                undefined.add(entry.name)
            else:
                defined.add(entry.name)
    need = undefined - defined - _UNDEFINED_ALLOWLIST
    need.add(INTERLAYER_BUFFER_SYMBOL)
    return sorted(need)
...
def _storage_model_name(symbol: str | None, suffix: str) -> str | None:
    prefix = "axon_model_"
    if not symbol or not symbol.startswith(prefix) or not symbol.endswith(suffix):
        return None
    return symbol[len(prefix) : -len(suffix)]


def derive_axon_model_token(model_symbol: str, metadata: ProbeMetadata) -> str:
    """Uppercase token for NRF_AXON_MODEL_<TOKEN>_PACKED_OUTPUT_SIZE re-exports."""
    from_storage = _storage_model_name(metadata.persistent_symbol, "_persistent_vars")
    if from_storage is None:
        from_storage = _storage_model_name(metadata.packed_output_symbol, "_packed_output_buf")
    if from_storage is not None:
        return sanitize_model_id(from_storage)
    if model_symbol.startswith("model_"):
        return sanitize_model_id(model_symbol[len("model_") :])
    raise ValueError(f"cannot derive Axon model name from model symbol {model_symbol!r}")


def render_keep_json(model_id: str, metadata: ProbeMetadata) -> str:
    payload = {
        "target": model_id,
        "keep_symbols": list(metadata.keep_symbols),
    }
...
def render_private_header(header_name: str, metadata: ProbeMetadata) -> str:
    ...
    if metadata.keep_symbols:
        lines.append(f"#define MODEL_OTA_AXON_KEEP_SYMBOL_COUNT {len(metadata.keep_symbols)}")
        for symbol in metadata.keep_symbols:
            lines.append(
                f"#define MODEL_OTA_AXON_SYM_HASH_{symbol} "
                f"0x{symbol_name_hash(symbol):08X}u"
            )
        lines.append("#define MODEL_OTA_AXON_SYM_HASH(sym) MODEL_OTA_AXON_SYM_HASH_##sym")
        ...
        lines.append("#define MODEL_OTA_AXON_KEEP_SYMBOL_COUNT 0")
        lines.append("#define MODEL_OTA_AXON_KEEP_REFS(X)")
    return "\n".join(lines) + "\n"


def render_public_header(model_id: str, metadata: ProbeMetadata) -> str:
    slot_token = sanitize_model_id(model_id)
    model_token = derive_axon_model_token(metadata.model_symbol, metadata)
    lines = [
        "/* Generated by axon_elf.py; do not edit. */",
        "#pragma once",
        f"/* Firmware capacities for OTA slot {model_id}. An image may require less. */",
        f"#define MODEL_OTA_AXON_{slot_token}_PACKED_OUTPUT_BYTES {metadata.packed_output_bytes}",
        f"#define MODEL_OTA_AXON_{slot_token}_PERSISTENT_VARS_CAP {metadata.persistent_cap}",
        "",
        f"/* Re-exported from the Axon model header, which an OTA-wired app does not compile. */",
        f"#ifndef NRF_AXON_MODEL_{model_token}_PACKED_OUTPUT_SIZE",
        f"#define NRF_AXON_MODEL_{model_token}_PACKED_OUTPUT_SIZE \\",
        f"\tMODEL_OTA_AXON_{slot_token}_PACKED_OUTPUT_BYTES",
        "#endif",
    ]
    return "\n".join(lines) + "\n"


def render_binding_table_header(symbols: Sequence[str]) -> str:
    hashes: dict[int, str] = {}
    for symbol in symbols:
        digest = symbol_name_hash(symbol)
        previous = hashes.get(digest)
        if previous is not None and previous != symbol:
            raise ValueError(
                f"symbol name hash collision between {previous!r} and {symbol!r} "
                f"(0x{digest:08X})"
            )
        hashes[digest] = symbol

    lines = [
        "/* Generated by axon_elf.py; do not edit. */",
        "#pragma once",
        f"#define MODEL_OTA_AXON_BINDING_SYMBOL_COUNT {len(symbols)}",
    ]
    for symbol in symbols:
        lines.append(
            f"#define MODEL_OTA_AXON_SYM_HASH_{symbol} 0x{symbol_name_hash(symbol):08X}u"
        )
    lines.append("#define MODEL_OTA_AXON_SYM_HASH(sym) MODEL_OTA_AXON_SYM_HASH_##sym")
    if symbols:
        lines.append("#define MODEL_OTA_AXON_BINDING_REFS(X) \\")
        for pos, symbol in enumerate(symbols):
            continuation = " \\" if pos + 1 < len(symbols) else ""
            lines.append(f"\tX({symbol}){continuation}")
    else:
        lines.append("#define MODEL_OTA_AXON_BINDING_REFS(X)")
    return "\n".join(lines) + "\n"
...
def load_keep_symbols(keep_json: Path) -> list[str]:
    ...
    if not isinstance(symbols, list) or not all(isinstance(entry, str) for entry in symbols):
        raise ValueError(f"{keep_json}: keep_symbols must be a JSON string array")
    return symbols


def merge_keep_symbols(keep_json_paths: Sequence[Path]) -> list[str]:
    merged: set[str] = set()
    for path in keep_json_paths:
        merged.update(load_keep_symbols(path))
...
def cmd_inspect(args: argparse.Namespace) -> int:
    ...
    public = render_public_header(args.model_id, metadata)
    ...
    if args.keep_json is not None:
        atomic_write_if_changed(args.keep_json, render_keep_json(args.model_id, metadata))
    return 0


def cmd_binding_table(args: argparse.Namespace) -> int:
    if not args.keep_json:
        raise ValueError("at least one --keep-json is required")
    for path in args.keep_json:
        if not path.is_file():
            raise ValueError(f"keep JSON not found: {path}")
    header = render_binding_table_header(merge_keep_symbols(args.keep_json))
    atomic_write_if_changed(args.output, header)
    return 0
...
def cmd_provide(args: argparse.Namespace) -> int:
    ...
    symbols: list[str] = collect_provide_symbols(objects)
...
def create_parser() -> argparse.ArgumentParser:
    ...
    inspect.add_argument(
        "--keep-json",
        type=Path,
        help="per-slot keep-symbol manifest for the device-wide binding table",
    )
    ...
    binding_table = sub.add_parser(
        "binding-table", help="merge per-slot keep JSON into one binding-table header"
    )
    binding_table.add_argument("--keep-json", type=Path, action="append", required=True)
    binding_table.add_argument("-o", "--output", type=Path, required=True)
    binding_table.set_defaults(func=cmd_binding_table)

    provide = sub.add_parser("provide", help="emit a PROVIDE() linker fragment")
    provide.add_argument("--object", type=Path, nargs="+", required=True)
