...
def symbol_name_hash(name: str) -> int:
    """FNV-1a over an ASCII symbol name; must match MODEL_OTA_AXON_SYM_HASH in generated headers."""
    value = 2166136261
...
def render_public_header(model_id: str, metadata: ProbeMetadata) -> str:
    ...
    contract = (
        "MODEL_OTA_CONTRACT_HASH_AXON("
        f"{metadata.persistent_required}, {metadata.packed_output_bytes})"
    )
    lines = [
        "/* Generated by axon_elf.py; do not edit. */",
        "#pragma once",
        ...
        f"#define MODEL_OTA_AXON_{token}_CONTRACT_HASH {contract}",
