...
def render_public_header(
    model_id: str, metadata: ProbeMetadata, partition_addr: int, edgeai: bool
) -> str:
    ...
    lines = [
        ...
        f"#define MODEL_OTA_AXON_{token}_PACKED_OUTPUT_BYTES {metadata.packed_output_bytes}",
        f"#define MODEL_OTA_AXON_{token}_PERSISTENT_VARS_CAP {metadata.persistent_cap}",
        f"#define MODEL_OTA_AXON_{token}_IMAGE_BASE 0x{partition_addr:08X}u",
        f"#define MODEL_OTA_AXON_{token}_KEEP_LABEL model_ota_axon_keep_{model_id}",
    ]
    if edgeai:
        ...
        lines.append(
            f"#define MODEL_OTA_AXON_{token}_CONTRACT_HASH "
            f"MODEL_OTA_CONTRACT_HASH_AXON(MODEL_OTA_AXON_{token}_IMAGE_BASE)"
        )
    return "\n".join(lines) + "\n"
...
def cmd_inspect(args: argparse.Namespace) -> int:
    ...
    public = render_public_header(args.model_id, metadata, args.partition_addr, args.edgeai)
