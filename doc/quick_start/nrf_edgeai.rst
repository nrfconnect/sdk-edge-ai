...
Follow the steps below to create your first ML model.
You can also check out the `Nordic Edge AI Lab Quick Start Guide`_ for a detailed walkthrough of the model training process.
...
* **Start with preloaded datasets** - Perfect for learning and experimentation, `Nordic Edge AI Lab`_ includes `sample datasets <Nordic Edge AI Lab preloaded datasets_>`_ you can use right away.
...
.. important::
   Remember that your dataset must include a ``target`` column that contains the labels or values for each data sample.
...
#. **Implement the AI functionality** using :ref:`nRF Edge AI API <nrf_edgeai_lib_api>`.
   ...
   * The :ref:`nRF Edge AI Samples <samples_nrf_edgeai_overview>` - demonstrate different AI use cases
