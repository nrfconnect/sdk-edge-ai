...
When test data isprovided, the compiler can also generate test vectors for on‑target verification.
...
* Compiler library - The compiler shared library performs the actual compilation and generates Axon object code.
  ...
  The compiler library provides platform-specific binaries:
...
Before you can run the compiler, you need to set up a Python environment with the required dependencies.
The executor supports Python ``v3.11.8``.

.. note::

  The examples below use the Windows command prompt. 
  Adjust commands as needed for your preferred environment and operating system.
...
#. Ensure the Conda scripts directory is added to your system ``PATH``, for example, :file:`C:/Users/<user>/AppData/Local/miniforge3/Scripts`.
...
.. list-table:: Advanced Parameters
   ...
   * - OP_RADIX
     - INT
     - Sets the output radix used when output quantization is disabled.

       The radix should be chosen based on the quantization range to maximize precision
       while maintaining sufficient margin to avoid saturation.
...
You can find the instructions for obtaining the required model files in the :file:`models/tinyml` directory.
