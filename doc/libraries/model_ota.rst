...
* :file:`samples/nrf_edgeai/regression/README.rst` ("Model-only OTA update") - both backends, folded into a real inference sample, enabled by default.
* :file:`samples/nrf_edgeai/classification/README.rst` ("Model-only OTA update") - both backends, opt-in (nRF54LM20 DK boards only).
* :file:`samples/nrf_edgeai/anomaly/README.rst` ("Model-only OTA update") - Neuton only (q16), opt-in (nRF54LM20 DK boards only) - the anomaly-detection task's ``MODEL_AVERAGE_EMBEDDING`` alongside the usual output scale.
...
A Neuton package's payload is just the model's own raw arrays (weights, activation weights, output scaling, neuron/link topology, activation type mask), concatenated in a fixed order, each one starting at the next 4-byte-aligned offset from the payload's own start, with no embedded absolute addresses anywhere.
Because nothing in the payload needs to know its own flash address, the on-device loader (``model_pkg_load_neuton()`` in :file:`model_pkg_neuton.c`) simply wires each section's flash address (found by offsetting from the start of the payload, skipping the same alignment padding the packer inserted) directly into a ``nrf_edgeai_model_neuton_t``, plus a caller-provided RAM scratch buffer for neuron activations sized by :kconfig:option:`CONFIG_MODEL_OTA_MAX_NEURONS`.
The alignment matters for q16/q8 packages: without it, an odd ``weights_num``/``neurons_num`` can leave a later section at an address the Neuton runtime's quantized inference kernels cannot access directly (a usage fault on real Cortex-M hardware), even though plain unaligned loads/stores are otherwise tolerated on those targets.
...
* :file:`package_model.py` - builds a Neuton package from a plain-JSON model description (see :file:`samples/nrf_edgeai/regression/src/nrf_edgeai_generated/Neuton/regression_v2.json` for an example).
  No ELF is needed, since a Neuton payload embeds no addresses.
  Useful for hand-edited/synthetic variants that have no corresponding generated source - :file:`regression_v2.json` is exactly that: a deliberately hand-tweaked "retrained" variant with no :file:`.c` counterpart.
* :file:`package_model_neuton.py` - builds a Neuton package directly from a Neuton codegen's generated model source (e.g. :file:`nrf_edgeai_generated/Neuton/nrf_edgeai_user_model.c` under any Neuton sample), regex-parsing its ``#define`` counts and ``static const`` coefficient arrays straight into a package - no hand-transcription into JSON, and (like :file:`package_model.py`) no ELF needed.
  ...
  ``MODEL_PARAMS_TYPE`` may be ``f32``, ``q16``, or ``q8``; the package header records which one so the on-device loader knows which ``nrf_edgeai_model_neuton_params_u`` member (and ``neurons_buf`` element type) to populate. The tool accepts every Neuton task: regression/anomaly-detection models carry their ``MODEL_OUTPUT_SCALE_MIN``/``MAX`` in the package (needed to decode output back into physical units); anomaly-detection models additionally carry ``MODEL_AVERAGE_EMBEDDING`` (needed by ``nrf_edgeai_decoded_output_anomaly_t``); classification models have neither array to begin with, since ``nrf_edgeai_decoded_output_classif_t`` has no scale concept, and are packaged with both sections empty instead.
...
* :file:`test_package_model_neuton.py` - host-side unit tests for the Neuton source parser, checking it extracts the expected coefficients from :file:`samples/nrf_edgeai/regression/src/nrf_edgeai_generated/Neuton/nrf_edgeai_user_model.c` and produces a valid package from them, plus its error paths (wrong precision, wrong task, missing arrays).
...
Both ``model_pkg_load_axon()`` and ``model_pkg_load_neuton()`` take their target partition as parameters (a flash-area ID and its memory-mapped base address - see :c:func:`PARTITION_ID` / :c:func:`PARTITION_ADDRESS`), rather than hard-coding a single ``model_storage`` label, precisely so that an application hosting several independent models of either backend (see :file:`applications/ww_kws` for Axon) can call the loader once per model, against each one's own partition.

Neuton/Axon alignment
************************

Both backends share the same header "shape" (magic, format version, model type, name, model version, size fields, CRC32), the same fail-closed loading behavior, and the same parameterized-by-partition loader signature, but are otherwise independent, and deliberately not aligned everywhere:

* **Build-time packaging is symmetric.** ``nrf_neuton_model_package()`` (:file:`lib/model_ota/cmake/nrf_neuton_model_package.cmake`) mirrors ``nrf_axon_model_stub()``: both run automatically as part of a normal ``west build`` and drop a ready-to-flash ``.bin``/``.hex`` straight in the build directory, with no separate manual packaging step for either backend's samples.
...
* **On-device loading is not symmetric.** Axon's loader performs no RAM copy of ``cmd_buffer``/``model_const`` at all (see "On-device loading (XIP)" above); Neuton's wires each array's flash address directly into the model instance too, but its neuron activation scratch buffer is necessarily RAM (Neuton recomputes it every inference; Axon has no equivalent).
...
#. To observe an update, hand-tweak a constant directly in the bundled generated source (for example a weight in :file:`src/nrf_edgeai_generated/Neuton/nrf_edgeai_user_model.c`), rebuild (``west build``, no ``-p`` needed), and flash the newly produced package the same way, again without touching the application image:

   .. code-block:: console

      west build -d build samples/nrf_edgeai/regression
      nrfutil device program --firmware build/regression/regression_model_pkg.hex --core Application \
        --options chip_erase_mode=ERASE_RANGES_TOUCHED_BY_FIRMWARE,reset=RESET_SYSTEM

   ``nrf_neuton_model_package()`` depends on that ``.c`` file, so an incremental build re-runs :file:`package_model_neuton.py` automatically whenever it changes - watch for ``model_ota: packaging regression (aq_regression, Neuton)`` again in the build log.
   The predicted/error values change on the next periodic reload or after a reset.

#. To package a different model instead (for example a freshly (re)trained one, kept outside this sample's own tree), run :file:`package_model_neuton.py` directly against its own generated source - no hand-transcription into JSON needed:
...
#. Repackage from :file:`samples/nrf_edgeai/regression/src/nrf_edgeai_generated/Neuton/regression_v2.json` (a hand-tweaked variant with no corresponding generated source) using :file:`package_model.py` instead:

   .. code-block:: console

      python3 tools/model_ota/package_model.py \
        samples/nrf_edgeai/regression/src/nrf_edgeai_generated/Neuton/regression_v2.json \
        -o model_v2 --dts build/regression/zephyr/zephyr.dts
...
Each sample/application additionally defines its own top-level toggle (:kconfig:option:`CONFIG_HELLO_AXON_MODEL_OTA`, :kconfig:option:`CONFIG_NRF_EDGEAI_REGRESSION_MODEL_OTA`, :kconfig:option:`CONFIG_NRF_EDGEAI_CLASSIFICATION_MODEL_OTA`, :kconfig:option:`CONFIG_NRF_EDGEAI_ANOMALY_MODEL_OTA`, :kconfig:option:`CONFIG_APP_MODEL_OTA`) that ``select``\ s the ones above, plus (for :file:`applications/ww_kws`, which needs an application-owned ``persistent_vars`` RAM array per model - see "Axon packages" above) :kconfig:option:`CONFIG_APP_WW_PERSISTENT_VARS_SIZE` / :kconfig:option:`CONFIG_APP_KWS_PERSISTENT_VARS_SIZE`, which must match the size the currently bundled model's own generated header declares.
...
See :file:`samples/axon/hello_axon/src/main.c`, :file:`samples/nrf_edgeai/regression/src/nrf_edgeai_generated/Neuton/nrf_edgeai_user_model.c` / :file:`nrf_edgeai_generated/Axon/nrf_edgeai_user_model.c` (their ``nrf_edgeai_load_user_model_90508()`` / ``_36025()`` functions), and :file:`applications/ww_kws/src/ww/nrf_edgeai_generated/nrf_edgeai_user_model.c` / :file:`src/kws/nrf_edgeai_generated/nrf_edgeai_user_model.c` for concrete examples of this pattern - the ``ww_kws`` ones are also the reference for wiring up a model that uses op extensions and/or ``persistent_vars``.

:file:`samples/nrf_edgeai/regression` additionally demonstrates one more refinement: instead of hand-transcribing the compiled-in fallback model separately from its "real" generated source (as :file:`applications/ww_kws` still does), its :file:`nrf_edgeai_generated/<Backend>/nrf_edgeai_user_model.c` is the actual generated model source, plus a mutable RAM model instance. With ``CONFIG_MODEL_OTA_AXON`` / ``CONFIG_MODEL_OTA_NEUTON`` disabled, the generated accessor (``nrf_edgeai_user_model_36025()`` / ``_90508()``) populates that instance from the compiled-in constants right there in the file; with it enabled, that same accessor name instead just returns the current instance as-is (only valid after a load), and a second function - ``nrf_edgeai_load_user_model_36025()`` / ``_90508()``, taking a flash area ID and partition address as parameters, so the very same generated code can serve more than one model instance, each backed by its own partition - calls straight into ``model_pkg_load_axon()`` / ``model_pkg_load_neuton()`` and returns the instance on success. :file:`nrf_edgeai_user_model.h`'s ``nrf_edgeai_user_model`` macro aliases to whichever of the two is right for the current config, so callers like ``main()`` stay agnostic to both the backend and whether OTA is enabled at all. So the compiled-in path and the OTA path can never drift apart by construction, and there is no separate hand-written file that needs to track the model's shape (element types, struct member names) to reach into it from outside. The compiled-in constants themselves (and the code that copies them into the mutable instance) are wrapped in ``#if !defined(CONFIG_MODEL_OTA_AXON)`` / ``NEUTON``, so an OTA build never links the model's weights into the app image at all - the whole point of model-only OTA is to keep those bytes out of the app partition, not just to add a second, flash-resident copy of them. See :file:`tools/model_ota/examples/hypothetical_ota_generator_output/` for how a similar split (into an always-generated file and an OTA-only one) could still make sense for a real generator that has no notion of OTA to begin with.
