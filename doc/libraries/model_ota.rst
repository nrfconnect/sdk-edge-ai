...
See :file:`samples/axon/hello_axon/src/main.c`, :file:`samples/nrf_edgeai/regression/src/nrf_edgeai_generated/Neuton/nrf_edgeai_user_model.c` / :file:`nrf_edgeai_generated/Axon/nrf_edgeai_user_model.c` (their ``nrf_edgeai_load_user_model_90508()`` / ``_36025()`` functions), and :file:`applications/ww_kws/src/ww/model_wiring_axon.c` / :file:`src/kws/model_wiring_axon.c` for concrete examples of this pattern - the ``ww_kws`` ones are also the reference for wiring up a model that uses op extensions and/or ``persistent_vars``.
