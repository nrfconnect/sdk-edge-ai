...
* Deployed machine learning model - A package generated by Edge Impulse Studio that contains your trained model's parameters and generated source files.
...
To integrate the Edge Impulse machine learning model into an |EAI| application, first prepare and deploy it for your embedded device.
...
To build the application with the integrated Edge Impulse model, you need a :file:`<model_name>.zip` archive with the deployed model, obtained in the :ref:`ug_edge_impulse_adding_preparing` step.
...
The :ref:`hello_ei_sample` sample integrates an Edge Impulse model archive through the ``CONFIG_EDGE_IMPULSE_PATH`` Kconfig option and CMake's ``FetchContent``.
