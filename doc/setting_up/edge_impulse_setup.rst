...
#. Create an `Edge Impulse studio account <Edge Impulse studio signup>`_ if you want to train and deploy your own machine learning models.
#. Follow the `Edge Impulse CLI installation guide`_ to install Edge Impulse command line tools.
   They include e.g. ``edge-impulse-data-forwarder`` which can be used to forward data from a board to |EIS| for training machine learning models.
...
These models can be run only on Axon-enabled devices, such as:
