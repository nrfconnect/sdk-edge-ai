...
/* set this to 0 to skip full model vectors and only execute single layer models */
#define END_TO_END_INFERENCE_ENABLED 1
...
static void unpack_vector32(int32_t *unpacked, const int32_t *packed,
	...
	uint16_t unpacked_stride, uint16_t batch_cnt)
{
	...
	for (int ch_ndx = 0; ch_ndx < (packed_dimensions->channel_cnt * batch_cnt); ch_ndx++) {
		for (int height_ndx = 0; height_ndx < packed_dimensions->height; height_ndx++) {
			...
			/*
			 * expected_output is packed, outpout isn't, so advance the
			 * output_ndx past the padding space
			 */
	...
	uint16_t unpacked_stride, uint16_t batch_cnt)
{
	...
	for (int ch_ndx = 0; ch_ndx < (packed_dimensions->channel_cnt * batch_cnt); ch_ndx++) {
		for (int height_ndx = 0; height_ndx < packed_dimensions->height; height_ndx++) {
			...
			/*
			 * expected_output is packed, output isn't,
			 * so advance the output_ndx past the padding space
			 */
			unpacked += extra_stride;
		}
	}
	...
	uint16_t unpacked_stride, uint16_t batch_cnt)
{
	...
	for (int ch_ndx = 0; ch_ndx < (packed_dimensions->channel_cnt * batch_cnt); ch_ndx++) {
		for (int height_ndx = 0; height_ndx < packed_dimensions->height; height_ndx++) {
			...
			/* expected_output is packed, outpout isn't, so advance the output_ndx
			 * past the padding space
			 */
			unpacked += extra_stride;
		}
	}
	...
	uint16_t unpacked_stride, uint16_t batch_cnt)
{
	...
	case 1:
		unpack_vector8((int8_t *)unpacked, (int8_t *) packed,
			packed_dimensions, unpacked_stride, batch_cnt);
		break;
	case 2:
		...
		for (int height_ndx = 0; height_ndx < packed_dimensions->height; height_ndx++) {
			for (int width_ndx = 0; width_ndx < packed_dimensions->width; width_ndx++) {
				...
				if (!error) {
					...
					result++;
					/* if this was on of the 1st
					 * 15 errors, display its location.
					 */
					if (result < 15) {
						nrf_axon_platform_printf(
							"output mismatch: %d != %d @ch %d,"
							"row %d, col %d\n",
							output_val, expected_val, ch_ndx,
							height_ndx, width_ndx);
					}
					if (error > max_error) {
						max_error = error;
						max_error_cnt = 1;
			...
			/* expected_output is packed, outpout isn't, so advance
			 * the output_ndx past the padding space
			 */
			output += extra_stride;
		}
	}
...
void nrf_axon_layer_model_results_to_buffer(
	nrf_axon_nn_compiled_model_layer_s * compiled_layer_model,
	int8_t *to_buffer)
{
...
/**
 * This function accepts a complete model input that has a single, external input, and performs
 * a synchronous(blocking) inference.
 * This function only works for complete models that expect a single, packed external input vector.
 * Inner layer models are not supported.
 */
	...
	const int8_t *packed_input, const int8_t **expected_output_list)
{
	...
	/*
	 * Note: by supplying output_buffer == NULL the output results will remain in the
	 * interlayer buffer after the axon reservation is freed. This is safe as long as
	 * it is guaranteed that no other threads are using Axon. This test code can't
	 * guarantee that the locally declared output_buffer is sized properly for the output.
	 */

	/* use the output buffer provisioned by the compiler if available, otherwise
	 * leave output in the interlayer buffer.
	 */
	int8_t *packed_output_ptr = compiled_model->packed_output_buf != NULL ?
		compiled_model->packed_output_buf :
		NULL;
	nrf_axon_result_e result = nrf_axon_nn_model_infer_sync(compiled_model,
		packed_input, packed_output_ptr);

	/*
	 * inference is complete but cannot release axon until output has either been consumed
	...
	result = test_compare_results(compiled_model, packed_output_ptr, expected_output_list);
...
/**
 * This function accepts a complete model input that has a single external input,
 * and performs a non-blocking inference (even though it just stalls while waiting for a result),
 * and returns the inferred label index.
 * This function only works for complete models that expect a single, packed external input vector.
 * Inner layer models are not supported.
 *
 * In asynchronous mode the driver needs to copy the input to the interlayer buffer only when the
 * model is about to run so as to not corrupt any on-going axon operations. Similarly, the
 * output needs to be copied from the interlayer buffer at model completion before the next model
 * is run. By supplying output_buffer to nrf_axon_nn_model_infer_async(), the driver will populate
 * it with the output results.
 *
 * Note that the driver will start the next job in the queue before invoking the current job's
 * callback. So it isn't safe to retrieve the output in the callback function.
 */
	...
	const int8_t *packed_input, const int8_t **expected_output_list)
{
	...
	nrf_axon_platform_set_profiling_gpio();
	/*
	 * set the input vector length to -1 to skip the length validation since we know
	 * the vector length is correct.
	 * We don't know a-priori if the local output buffer is sufficient to hold the output
	 * results. If it is, have the driver copy the output to it. This method guarantees
	 * that the output will not be overwritten by another inference if there are multiple
	 * users of axon.
	 */
	int8_t *packed_output_ptr = compiled_model->packed_output_buf != NULL ?
		compiled_model->packed_output_buf :
	...
	if (test_compare_results(model_wrapper->compiled_model,
	...
	uint32_t *test_fail_cnt)
{
	...
	/* bind the full model for async operation since every other
	 * vector will be in async mode.
	 */
	...
	for (int vector_ndx = 0; vector_ndx < test_vectors->full_model_vector_count;
	...
	const nrf_axon_nn_model_test_info_s *test_vectors)
{
	...
	for (model_ndx = 0; model_ndx < models_count; model_ndx++) {
		...
		/* only count layers that have an associated model
		 * (ie, not persistent variable layers)
		 */
	...
	uint16_t layer_cnt)
{
	...
	the_struct->full_model_input_vectors = full_model_input_vectors;
	the_struct->full_model_expected_output_vectors = full_model_expected_output_vectors;
	the_struct->full_model_vector_count = full_model_vector_count;
	the_struct->layer_vectors = layer_vectors;
	the_struct->layer_cnt = layer_cnt;
}
